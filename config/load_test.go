...
func TestDefaults(t *testing.T) {
	...
	if got, want := conf.HetznerCloud.ServerType, "cx11"; got != want {
		t.Errorf("Want default DRONE_HETZNERCLOUD_TYPE of %s, got %s", want, got)
	}
}
...
func TestLoad(t *testing.T) {
	environ := map[string]string{
		...
		"DRONE_CADVISOR_DISABLE":        "true",
		"DRONE_DATABASE_DRIVER":         "mysql",
		"DRONE_DATABASE_DATASOURCE":     "user:password@/dbname",
		"DRONE_CLOUDINIT_PATH":          "/path/to/cloud-init",
	...
	if err := json.Unmarshal(jsonConfig, &b); err != nil {
